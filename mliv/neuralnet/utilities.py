...
def log_metrics(Z, S, T, Y, Z_val, S_val, T_val, Y_val, T_test, learner_s, learner, adversary, epoch, writer, true_of_T=None, true_of_T_test=None, loss='moment'):
    y_pred = learner_s(S) + learner(T).sum(dim=1)
    y_pred_val = learner_s(S_val) + learner(T_val).sum(dim=1)
    if loss == 'moment':
        writer.add_scalar('moment',
                          torch.mean((Y - y_pred)
                                     * adversary(Z).sum(dim = 1)),
                          epoch)

        writer.add_scalar('moment_val',
                          torch.mean((Y_val - y_pred_val)
                                     * adversary(Z_val).sum(dim = 1)),
                          epoch)
    if loss == 'kernel':
...
def plot_results(est, S_test, T_test, true_of_T_test, fname=None, ind=0):
    point, lb, ub = est.predict(S_test, T_test, burn_in=0, alpha=0.2)
    point_final = est.predict(S_test, T_test, model='final')
    point_earlystop = est.predict(S_test, T_test, model='earlystop')

    truth = true_of_T_test.cpu().numpy()

    X_test = S_test + T_test.sum(dim = 1)

    plt.figure(figsize=(15, 5))
    plt.subplot(1, 2, 1)
    plt.plot(X_test[:, ind].cpu(), point, label='avg')
    plt.fill_between(X_test[:, ind].cpu(),
                     lb.flatten(), ub.flatten(), alpha=.2)
    plt.plot(X_test[:, ind].cpu(), point_final, label='last')
    #plt.plot(T_test[:, ind].cpu(), point_earlystop, label='earlystop')
    plt.plot(X_test[:, ind].cpu(), truth, label='true')
    plt.legend()
    plt.subplot(1, 2, 2)
    ...
    R2fin = 1 - np.mean((truth.flatten() - point_final.flatten())
                        ** 2) / np.var(truth.flatten())
    R2earlystop = 1 - np.mean((truth.flatten() - point_earlystop.flatten())**2) / np.var(truth.flatten())
