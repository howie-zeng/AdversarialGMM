...
class _BaseAGMM:
    ...
    def _pretrain(self, Z, S, X, Y,
        ...
        self.learner_s = self.learner_s.to(device)
        ...
        beta1 = 0.
        self.optimizerD_s = OAdam(add_weight_decay(self.learner_s, learner_l2),
        ...
        for epoch in range(n_epochs):
            ...
            for it, (zb, sb, xb, yb) in enumerate(self.train_dl):
                ...
                if (it % train_learner_every == 0):
                    self.learner_s.train()
                    self.learner.train()
                    pred = self.learner_s(sb) + self.learner(xb).sum(dim=1)
                    test = self.adversary(zb).sum(dim=1)

                    D_loss = torch.mean(
                        (yb - pred) * test) + ols_weight * torch.mean((yb - pred)**2)
                    self.optimizerD.zero_grad()
                    self.optimizerD_s.zero_grad()
                    D_loss.backward()
                    self.optimizerD_s.step()
...
class AGMM(_BaseSupLossAGMM):
    ...
    def __init__(self, learner, learner_s, adversary):
        """
        Parameters
        ----------
