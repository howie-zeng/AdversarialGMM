...
class _BaseAGMM:
    ...
    def _pretrain(self, Z, S, X, Y,
        ...
        if add_sample_inds:
            ...
            self.train_ds = TensorDataset(Z, S, X, Y, sample_inds)
        else:
            self.train_ds = TensorDataset(Z, S, X, Y)
    ...
    def predict(self, S, T, model='avg', burn_in=0, alpha=None):
        """
        Parameters
        ----------
        T : treatments
        ...
        if model == 'avg':
            ...
            T_preds_total = np.array([torch.load(os.path.join(self.model_dir,
                                                      "epoch{}".format(i)))(S, T).cpu().data.numpy()
                              for i in np.arange(burn_in, self.n_epochs)])
            T_preds_total = T_preds_total.squeeze()
            if alpha is None:
                T_preds= np.mean(T_preds_total, axis=0)
                preds = T_preds
                return preds
            else:
                preds = np.mean(T_preds_total, axis=0),\
                    np.percentile(T_preds_total, 100 * alpha / 2, axis=0),\
                    np.percentile(T_preds_total, 100 * (1 - alpha / 2), axis=0)
                return preds
        if model == 'final':

            T_preds = torch.load(os.path.join(self.model_dir,
                                           "epoch{}".format(self.n_epochs - 1)))(S, T).cpu().data.numpy()
            preds = T_preds
            return preds
        if model == 'earlystop':
            return torch.load(os.path.join(self.model_dir,
                                           "epoch{}".format(self.n_epochs//5)))(S, T).cpu().data.numpy()
        if isinstance(model, int):

            T_preds = torch.load(os.path.join(self.model_dir,
                                           "epoch{}".format(model)))(S, T).cpu().data.numpy()
            preds = T_preds
            return preds
...
class _BaseSupLossAGMM(_BaseAGMM):
    ...
    def fit(self, Z, S, X, Y,
        ...
        Z, S, X, Y = self._pretrain(Z, S, X, Y,
                                 learner_l2, adversary_l2, adversary_norm_reg,
                                 learner_lr, adversary_lr, n_epochs, bs, train_learner_every, train_adversary_every,
        ...
        for epoch in range(n_epochs):
            ...
            for it, (zb, sb, xb, yb) in enumerate(self.train_dl):

                zb, sb, xb, yb = map(lambda x: x.to(device), (zb, sb, xb, yb))

                if (it % train_learner_every == 0):
                    self.learner.train()
                    pred = self.learner(sb, xb)
